...
def gerar_pdf_relatorio(dados_relatorio, data_inicio, data_fim, resumo):
    ...
    # --- LÓGICA ATUALIZADA PARA O PDF ---
    
    # 1. Cabeçalho da tabela
    table_data = [['Data', 'Cliente', 'Produto(s)', 'Qtd', 'Venda Total', 'Pagamento', 'Parc.', 'Status']]
    
    # 2. Lista para guardar os estilos de formatação
    style_commands = [
    ...
    # 3. Processa as linhas de dados
    for i, row_data in enumerate(dados_relatorio):
        # Adiciona a linha de dados formatada à tabela
        table_data.append([
            row_data['data'],
            row_data['cliente'],
            row_data['produto'],
            row_data['qtd'],
            format_currency(row_data['venda_total']),
            row_data['pagamento'],
            row_data['parcelas'],
            row_data['status']
        ])
        
        # 4. Adiciona um estilo de formatação se a venda for estornada
        if row_data['status'] == 'Estornada':
            row_index = i + 1  # +1 porque a linha 0 é o cabeçalho
            style_commands.append(('TEXTCOLOR', (0, row_index), (-1, row_index), colors.grey))
            style_commands.append(('BACKGROUND', (0, row_index), (-1, row_index), colors.lightgrey))

    # 5. Cria a tabela com os dados e os estilos
    t = Table(table_data)
