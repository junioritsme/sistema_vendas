...
class RelatorioVendasTab(ttk.Frame):
    ...
    def gerar_relatorio(self):
        ...
        for row in relatorio:
            id_venda = (row[0], row[1], row[6], row[7], row[8], row[9])
    ...
    def gerar_pdf(self):
        messagebox.showinfo("PDF", "A geração de PDF precisa ser adaptada para o novo formato de relatório.")

class CaixaFrame(ttk.Frame):
    def __init__(self, parent, app_ref): # <-- ALTERAÇÃO AQUI
        super().__init__(parent)
        self.app = app_ref # <-- ALTERAÇÃO AQUI
...
# É definida por último, pois precisa que as classes das abas já existam.
# =================================================================================
class App(tk.Tk):
    def __init__(self):
        super().__init__()
